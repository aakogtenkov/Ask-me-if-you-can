...
Теперь необходимо загрузить обученную версию из файла. В нашем случае она хранится под названием test_network:
    ...
    load -m test_network
Поскольку загрузка сети вызывает вначале построение графа вычислений, то время загрузки может быть до несколькиз минут, в зависимости от мощности компьютера. После того, как программа выведет строку 'Done!', наша сеть полностью готова к работе. Осталось только запустить ее в режиме тестирования. Для этого введем команду
...
A = softmax(W_a * GRU(q, m)), где W_a - это некоторая матрица, GRU(x, y) - это применение матриц рекуррентной сети GRU для входа x и текущего состояния сети y. Дописав несколько строк кода в модуль, можно добиться генерации последовательности векторов в качестве ответа (см. статью).
...
## Заключение (выжимка из всего написанного выше) и результаты

В рамках проекта была создана, обучена и протестирована нейронная сеть, генерирующая ответ по тексту и вопросу. Обучение и тестирование происходило на bAbI-задании. А именно, были выбраны 16 из 20 заданий bAbI-task'a, и была обучена нейронная сеть, способная отвечать на все эти 16 типов вопросов. Качество сети на отложенной выборке составило более 90% при минимальном качестве на одном типе вопросов более 70%.

Однако оказалось, что на реальных текстах и вопросах (которые составляются человеком на основе заданий bAbI-task'a) сеть показывает гораздо худший результат. Связано это, скорее всего, с тем, что она переобучается: специфика сгенерированных компьютером заданий bAbI очень сильна. 
...
Также была сделана обертка в виде консольного интерфейса для людей, которые незнакомы с Питоном или не хотят напрямую писать код для работы с реализованным классом сети и другими функциями. 
