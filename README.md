...
A = GRU([softmax(W_a * m), q], m), где W_a - это некоторая матрица, [] обозначает конкатенацию, GRU(x, y) - это применение матриц рекуррентной сети GRU для входа x и текущего состояния сети y. Дописав несколько строк кода в модуль, можно добиться генерации последовательности векторов в качестве ответа (см. статью).
