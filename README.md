...
Довольно известным фактом является то, что многие компании (в частности, Google, Yandex и другие) занимаются проблемой распознавания речи, картинок и т.д. Одним из таких разделов в области распознавания является распознавание фраз, выделения из них сути и генерация ответа, если таковой известен системе. Примерами таких алгоритмов являются разнообразные чатботы. Также решение этой задачи позволяет быстро находить необходимую информацию в тексте (например, в википедии) машинным образом.
...
Приложение тестировалось на bAbI-task задании, и в текущей конфигурации оно заточено именно под него. А именно, при работе нейронной сети при кодировании слов используется one-hot кодирование, то есть скорость работы и количество потребляемой памяти линейно зависит от количества слов в текстах. В bAbI-task задании используется всего около 200 слов, однако не предполагается использование данного приложения в текущем виде на данных с большим количеством слов. Для использования на больших данных необходима модификация функций обработки текста и использование word2vec (реализация word2vec на данных из википедии была выполнена в одной из лабораторных).
...
**Принцип работы приложения (туториал)**

Прежде всего, для обучения нейронной сети нужно подготовить данные. Допустим, наши данные для обучения записаны в обычном формате (см. "Поддерживаемые форматы данных") в файле train.txt. Используем команду prepare data (см. список команд).

    prepare data train.txt
    
В результате выполнения этой строки приложением будет создан файл 'train_prepared.txt' с подготовленными данными. Далее нам необходимо по этим данным создать словарь. Выполним команду

    prepare dictionary train_prepared.txt dict.txt

Итогом работы этой строки будет создание файла dict.txt, в котором будет содержаться словарь слов файла train_prepared.txt.
...
2) prepare. У этой команды есть множество различных вариаций. Использование этой команды позволяет приводить исходные текстовые файлы к виду, который используется для обучения и тестирования нейронной сети.
      ...
      a) prepare data \<filename>. Эта команда приводит файл \<filename> с текстом в обычном формате (см. "Поддерживаемые форматы") к виду, который использует нейронная сеть, и сохраняет в файл, название которого генерируется автоматически исходя из названия \<filename>.
      ...
      c) prepare babidata \<filename>. Эта команда приводит файл \<filename> с текстом в bAbI-формате (см. "Поддерживаемые форматы") к виду, который использует нейронная сеть, и сохраняет в файл, название которого генерируется автоматически исходя из названия \<filename>.
