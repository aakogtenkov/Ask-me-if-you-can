...
* НА ДАННЫЙ МОМЕНТ РЕПОЗИТОРИЙ НАХОДИТСЯ В СОСТОЯНИИ РАЗРАБОТКИ, ПОЭТОМУ ИНФОРМАЦИЯ В README МОЖЕТ НЕ СООТВЕТСТВОВАТЬ РЕАЛЬНОЙ.
...
В самом начале необходимо скачать код проекта askme.py из этого репозитория и файлы test.txt, dict.txt, test_network.index, test_network.meta, test_network.data-00000-of-00001. Перейдем в папку с кодом и файлами и запустим приложение с помощью консольной команды
...
Теперь создадим нейронную сеть.
    ...
    create model
Диалоговое окно попросит нас последовательно ввести различные параметры нейронной сети. В качестве максимального количества предложений (самый первый аргумент) введем 58, все последующие константы оставим со значениями по умолчанию, вводя 0. Таким образом, в консоли будет отображено

    Please enter the following constants:

    Maximum number of sentences: 58
    Max size of dictionary (you can skip this step by typing 0 if you already have loaded dictionary): 0
    Typical size of hidden layers (you can skip this step by typing 0): 0
    Number of passes through data in network (you can skip this step by typing 0): 0
    Creating model. Please, wait...
Создание сети может продолжаться от десяти секунд до минуты в зависимости от мощности компьютера. Когда программа выведет 'Done!', мы можем продолжать вводить команды. После сделанной инициализации сети необходимо загрузить обученную версию из файла. В нашем случае она хранится под названием test_network:

    load -m test_network
После того, как программа выведет строку 'Done!', наша сеть полностью готова к работе. Осталось только запустить ее в режиме тестирования. Для этого введем команду
...
4) load. Эта команда позволяет загружать модели и словари. 
      ...
      a) load model \<name>. Эта команда позволяет загрузить модель с названием \<name>. Важно отметить, что одновременно приложение может хранить в оперативной памяти информацию только об одной модели. Загрузка модели затрет всю информацию о предыдущей модели. Также все параметры загружаемой модели должны совпадать с параметрами созданной модели.
...
Подробное описание нейронной сети содержится в файле Net description.pdf. Это связано с тем, что в github не поддерживает формулы в readme. Ниже приводится краткое описание.
...
A = softmax(W^{(a)} \cdot GRU(q, m)), где W_a - это некоторая матрица, GRU(x, y) - это применение матриц рекуррентной сети GRU для входа x и текущего состояния сети y. Дописав несколько строк кода в модуль, можно добиться генерации последовательности векторов в качестве ответа (см. статью).
